...
# Need to implement here
  ...
  padded_shape = (
      ...
      img.shape[1] + 2 * pad_amount
  )
  padded_img = np.zeros(padded_shape, dtype=img.dtype)
  padded_img[pad_amount:padded_shape[0] - pad_amount, pad_amount:padded_shape[1] - pad_amount] = img
  padded_img[:pad_amount, pad_amount:-pad_amount] = img[0]
  ...
  padded_img[-pad_amount:, :pad_amount] = img[-1, 0]
...
if __name__ == '__main__':
  img_noise = "noise.png" # <- need to specify the path to the noise image
  img_gt = "ori_img.png" # <- need to specify the path to the gt image
